...
class PythonFileService:
    ...
    def create_python_file(
        ...
        tenant = normalize_tenant(
            tenant or (get_user_tenant(user) if user else TENANT_PROJECT)
        )
        ...
        existing_file = PythonFile.objects.filter(
            file_hash=file_hash, tenant__in=tenant_query_values(tenant)
        ).first()
    ...
    def update_file_content(self, python_file, content):
        ...
        if python_file.file:
            try:
                ...
                print(
                    f"Warning: Failed to update Django file field for {python_file.name}: {e}"
                )
    ...
    def _update_nodes_folder_file(self, python_file, content):
        ...
        try:
            ...
            print(
                f"Warning: Failed to update nodes folder file for {python_file.name}: {e}"
            )
