...
class PythonFileCopyView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            for file_id in file_ids:
                try:
                    # 元ファイルを取得
                    original_file = get_object_or_404(PythonFile, pk=file_id, is_active=True)
                    ...
                    copied_file = PythonFile(
                        name=copy_name,
                        description=f"Copy of {original_file.description}" if original_file.description else f"Copy of {original_file.name}",
                        file_content=original_file.file_content,
                        uploaded_by=request.user if request.user.is_authenticated else None,
                        node_classes=original_file.node_classes.copy() if original_file.node_classes else {},
                    ...
                    if original_file.file:
                        try:
                            ...
                            if not file_copy_name.endswith('.py'):
                                file_copy_name = f"{os.path.splitext(file_copy_name)[0]}.py"
                            ...
                            logger.warning(f"Could not copy file content for {file_id}: {e}")
            ...
            status_code = status.HTTP_201_CREATED if copied_files else status.HTTP_400_BAD_REQUEST
    ...
    def _copy_by_filename(self, request, source_filename, target_filename):
        ...
        try:
            ...
            copied_file = PythonFile(
                ...
                analysis_error=original_file.analysis_error,
                file_size=len(updated_content.encode('utf-8')) if updated_content else 0,
            )
    ...
    def _update_class_names_in_code(self, source_code, target_filename):
        ...
        class_name = ''.join(word.capitalize() for word in re.split(r'[_\-]', base_name))
        ...
        # 複数行対応でクラス定義を置換
        updated_code = re.sub(class_pattern, replace_class_name, updated_code, flags=re.MULTILINE)
    ...
    def _update_node_classes(self, original_node_classes, target_filename):
        ...
        # ファイル名をPascalCaseのクラス名に変換
        new_class_name = ''.join(word.capitalize() for word in re.split(r'[_\-]', base_name))

        updated_node_classes = {}
        
        for class_name, class_info in original_node_classes.items():
            # クラス名を新しい名前に変更
            updated_class_info = class_info.copy() if isinstance(class_info, dict) else class_info
...
class PythonFileParameterUpdateView(APIView):
    ...
    def put(self, request):
        ...
        try:
            ...
            parameter_field = data.get('parameter_field', 'value')  # 'value', 'default_value', 'constraints'
            ...
            logger.info(f"Parameter '{parameter_key}.{parameter_field}' updated and re-analyzed for file {python_file.name}")
    ...
    def _update_parameter_in_source_code(self, source_code, parameter_key, parameter_field, parameter_value):
        ...
        if parameter_field == 'value':
            ...
            updated_code = re.sub(variable_pattern, replace_variable_assignment, updated_code, flags=re.MULTILINE)

            # パターン2: 辞書内の値（例: {"record_from_population": 100}）
            dict_pattern = rf'(["\']){re.escape(parameter_key)}\1\s*:\s*[^,\}}]+'
            
            def replace_dict_value(match):
                quote = match.group(1)
                formatted_value = self._format_value_for_python(parameter_value)
                return f'{quote}{parameter_key}{quote}: {formatted_value}'

            updated_code = re.sub(dict_pattern, replace_dict_value, updated_code)

            # パターン3: 関数呼び出しの引数（例: func(record_from_population=100)）
            func_arg_pattern = rf'(\b{re.escape(parameter_key)}\s*=\s*)[^,\)]*'
            
            def replace_func_arg(match):
                prefix = match.group(1)
                formatted_value = self._format_value_for_python(parameter_value)
                return f'{prefix}{formatted_value}'

            updated_code = re.sub(func_arg_pattern, replace_func_arg, updated_code)
    ...
    def _update_parameter_metadata_in_code(self, source_code, parameter_key, field_name, field_value):
        ...
        if not self._parameter_exists_in_parameters_dict(source_code, parameter_key):
            ...
            logger.info(f"Parameter '{parameter_key}' not found in parameters dict, skipping")
            return source_code
        
        logger.info(f"Starting update for parameter '{parameter_key}', field '{field_name}'")
        
        # より確実なアプローチ: 文字列の直接置換ではなく、AST解析的なアプローチを使用
        return self._update_specific_parameter_field(updated_code, parameter_key, field_name, field_value)

    def _update_specific_parameter_field(self, source_code, parameter_key, field_name, field_value):
        ...
        for pattern in exact_patterns:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    logger.info(f"Found exact match for '{parameter_key}' using pattern: {pattern}")
                    break
        ...
        if end_pos == -1:
            logger.error(f"Could not find matching closing paren for parameter '{parameter_key}'")
            return source_code
        ...
        if updated_content != param_content:
            updated_code = (
                source_code[:start_pos + 1] + 
                updated_content + 
                source_code[end_pos:]
            )
            logger.info(f"Successfully updated parameter '{parameter_key}' field '{field_name}'")
        else:
            logger.info(f"No changes needed for parameter '{parameter_key}' field '{field_name}'")
    ...
    def _update_field_in_parameter_content(self, param_content, field_name, field_value):
        ...
        if field_match:
            ...
            logger.info(f"Field position: start={field_start}, value_start={value_start}, value_end={value_end}")
    ...
    def _parameter_exists_in_parameters_dict(self, source_code, parameter_key):
        ...
        param_key_pattern = rf'["\'{parameter_key}\"\']\s*:\s*ParameterDefinition'
        
        return bool(re.search(param_key_pattern, params_content))
